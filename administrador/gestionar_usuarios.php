...
// Esta línea es opcional si JavaScript siempre carga la tabla.
// Se mantiene como un fallback o para depuración inicial.
...
<link rel="icon" href="imagenes/AB.ico" type="image/x-icon">
    ...
    <style>
        ...
        .cancel-button {
            ...
            font-size: 18px;
            width: 100%;
            margin-top: 10px; /* Space between submit and cancel */
            transition: background-color 0.3s ease;
        }
        .cancel-button:hover {
    ...
    <div class="container-wrapper">
        ...
        <div class="main-content">
            ...
            <div class="form-container">
                ...
                <form id="form-usuario"> <input type="hidden" id="user-id" name="id"> <label for="nombre">Nombre:</label>
                    <input type="text" id="nombre" name="nombre" required placeholder="Ingrese el nombre completo">

                    <label for="clave">Clave:</label>
                    <input type="password" id="clave" name="clave" placeholder="Ingrese la clave (dejar vacío para no cambiar en edición)">
                    ...
                    <input type="submit" id="submit-button" value="Registrar Usuario">
                    <button type="button" id="cancel-button" class="cancel-button" style="display: none;">Cancelar Edición</button> </form>
            </div>
            <div class="user-list">
                ...
                <table class="data-table">
                    ...
                    <tbody id="usuarios-table-body">
                        ...
                        // Esto se mantendrá como un fallback, pero loadUsers() en JS es la fuente principal
...
<style>
    ...
    // *** INICIO: Funciones de Interacción con la Tabla ***
    ...
    async function loadUserForEdit(userId) {
        try {
            ...
            const response = await fetch(`api/users/get_user_by_id.php?id=${userId}`); // Ruta ajustada
            if (!response.ok) {
            ...
            // No rellenamos la clave por seguridad, el campo queda en blanco
    ...
    // Función para eliminar usuario (tu función actual, solo reconfirmando las rutas)
    async function deleteUser(userId) {
        try {
            const formData = new FormData();
            formData.append('id', userId);
            // Ruta ajustada para la API
            const response = await fetch('api/users/delete_user.php', { // Ruta ajustada
                method: 'POST',
                body: formData
            });
    ...
    // Función para cargar/recargar usuarios (tu función actual, solo reconfirmando las rutas)
    async function loadUsers() {
        try {
            // Ruta ajustada para la API
            const response = await fetch('api/users/get_users.php'); // Ruta ajustada
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`HTTP error! status: ${response.status} - ${errorText}`);
            }
    ...
    // *** FIN: Funciones de Interacción con la Tabla ***

    document.addEventListener('DOMContentLoaded', function() {
        // Manejo del formulario de registro de usuario (ya lo tenías)
        document.getElementById('form-registro-usuario').addEventListener('submit', async function(e) {
            e.preventDefault();
            const formData = new FormData(this);
            // Añadir un campo oculto para indicar que es un registro (no una edición)
            // Esto es útil si process_user.php discrimina entre crear y actualizar
            formData.append('action', 'register');

            try {
                const response = await fetch('api/users/process_user.php', { // Ruta ajustada
                    method: 'POST',
                    body: formData
                });
                const result = await response.json();
        ...
        // Manejo del formulario de edición de usuario (NUEVO)
